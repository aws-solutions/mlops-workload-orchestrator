...
def configure_multi_account_parameters_permissions(
    ...
    blueprint_repository_bucket.add_to_resource_policy(
        s3_policy_read(
            [blueprint_repository_bucket.bucket_arn, f"{blueprint_repository_bucket.bucket_arn}/*"],
            principals,
        )
    )
    ...
    model_registry.add_property_override(
        ...
        model_package_group_policy(
            ...
            [dev_account_id.value_as_string, staging_account_id.value_as_string, prod_account_id.value_as_string],
        ),
    )

    # add environment variables to orchestrator lambda function
    orchestrator_lambda_function.add_environment(key="IS_DELEGATED_ADMIN", value=is_delegated_admin.value_as_string)
    orchestrator_lambda_function.add_environment(key="DEV_ACCOUNT_ID", value=dev_account_id.value_as_string)
    orchestrator_lambda_function.add_environment(key="DEV_ORG_ID", value=dev_org_id.value_as_string)
    orchestrator_lambda_function.add_environment(key="STAGING_ACCOUNT_ID", value=staging_account_id.value_as_string)
    orchestrator_lambda_function.add_environment(key="STAGING_ORG_ID", value=staging_org_id.value_as_string)
    orchestrator_lambda_function.add_environment(key="PROD_ACCOUNT_ID", value=prod_account_id.value_as_string)
