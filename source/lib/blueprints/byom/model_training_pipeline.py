...
class TrainingJobStack(core.Stack):
    def __init__(self, scope: core.Construct, id: str, training_type: str, **kwargs) -> None:
        ...
        self.encrypt_inter_container_traffic = pf.create_encrypt_inner_traffic_parameter(self)
        self.max_runtime_per_training_job_in_seconds = pf.create_max_runtime_per_job_parameter(self)
        ...
        self.kms_key_arn_provided = cf.create_attribute_provided_condition(self, "KMSProvided", self.kms_key_arn)
        ...
        self.training_attributes = dict(
            ...
            validation_data=core.Fn.condition_if(
                self.validation_data_provided.logical_id, self.validation_data.value_as_string, core.Aws.NO_VALUE
            ).to_string(),
            ...
            compression_type=core.Fn.condition_if(
                self.compression_type_provided.logical_id, self.compression_type.value_as_string, core.Aws.NO_VALUE
            ).to_string(),
            data_input_mode=self.data_input_mode.value_as_string,
            data_record_wrapping=core.Fn.condition_if(
                self.data_record_wrapping_provided.logical_id,
                self.data_record_wrapping.value_as_string,
                core.Aws.NO_VALUE,
            ).to_string(),
            attribute_names=core.Fn.condition_if(
            ...
            kms_key_arn=core.Fn.condition_if(
                self.kms_key_arn_provided.logical_id, self.kms_key_arn.value_as_string, core.Aws.NO_VALUE
            ).to_string(),
        ...
        if training_type == "HyperparameterTuningJob":
            ...
            self.hyperparameters_ranges = pf.create_hyperparameters_range_parameter(self)
        ...
        invoke_lambda_custom_resource = self._create_invoke_lambda_custom_resource(
            function_name=training_lambda.function_name, function_arn=training_lambda.function_arn
        )
    ...
    def _create_model_training_lambda(self):
        ...
        # update the training attributes
        self.training_attributes.update(
            {"sm_layer": sm_layer, "kms_key_arn_provided_condition": self.kms_key_arn_provided}
        )

        # create training job lambda
        training_lambda = model_training_job(scope=self, id="ModelTrainingLambda", **self.training_attributes)
