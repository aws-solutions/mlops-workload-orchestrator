...
from lib.blueprints.byom.pipeline_definitions.sagemaker_model_registry import (
    create_sagemaker_model_registry,
)
from lib.blueprints.byom.pipeline_definitions.cdk_context_value import (
...
class MLOpsStack(core.Stack):
    def __init__(
        self, scope: core.Construct, id: str, *, multi_account=False, **kwargs
    ) -> None:
        ...
        # add S3 Server Access Logs Policy
        access_logs_bucket.add_to_resource_policy(
            iam.PolicyStatement(
                ...
                conditions={
                    ...
                    "StringEquals": {"aws:SourceAccount": core.Aws.ACCOUNT_ID},
                },
            )
        )
        ...
        unique_id = core.Fn.select(
            0, core.Fn.split("-", create_id_function.get_att_string("UUID"))
        )
        ...
        metrics_mapping = core.CfnMapping(
            self, "AnonymousData", mapping={"SendAnonymousData": {"Data": "Yes"}}
        )
        metrics_condition = core.CfnCondition(
            self,
            "AnonymousDatatoAWS",
            expression=core.Fn.condition_equals(
                metrics_mapping.find_in_map("SendAnonymousData", "Data"), "Yes"
            ),
        )
        ...
        send_data_cr_properties = {
            ...
            "IsDelegatedAccount": is_delegated_admin
            if multi_account
